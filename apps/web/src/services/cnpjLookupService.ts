/**
 * CNPJ lookup service — busca dados publicos da empresa via BrasilAPI.
 *
 * Endpoint: https://brasilapi.com.br/api/cnpj/v1/{cnpj}
 * Sem autenticacao, gratuito, public CORS habilitado.
 *
 * Fallback: ReceitaWS (CORS proxy precisa) — nao implementado por padrao.
 */
...
export async function lookupCnpj(cnpj: string): Promise<CnpjData> {
  ...
  try {
    ...
    throw new CnpjNetworkError(
      `Não foi possível conectar à BrasilAPI: ${(err as Error).message}`,
    )
  }
  if (response.status === 404) {
    throw new CnpjNotFoundError('CNPJ não encontrado na base da Receita.')
  }
  if (!response.ok) {
    throw new CnpjNetworkError(`BrasilAPI HTTP ${response.status}`)
  }
  const data = (await response.json()) as BrasilApiResponse
  if (data.type === 'not_found') {
    throw new CnpjNotFoundError(data.message || 'CNPJ não encontrado.')
  }
  return {
    cnpj: digits,
