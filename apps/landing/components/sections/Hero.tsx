...
import { ArrowRight, Check, PlayCircle, Sparkles, Lock, Activity } from 'lucide-react'
...
const partners = ['SGBR BI', 'Bling', 'Tiny ERP', 'Omie', 'Custom API', 'CSV / Excel', 'Sankhya', 'TOTVS']
    ...
    >
      ...
      <Container size="wide" className="relative">
        ...
        >
          ...
          <a href="#cta" className="text-sm text-[var(--color-brand)] hover:underline inline-flex items-center gap-1">
            Pedir convite <ArrowRight className="size-3.5" />
          </a>
        </motion.div>
        ...
        >
          ...
          <span className="text-gradient-animated">Precisa entender o que já tem.</span>
        </motion.h1>
        ...
        >
          ...
          <motion.div
            ...
            className="hidden md:flex float absolute -right-4 lg:-right-10 top-32 lg:top-40 z-10 flex-col gap-3 rounded-2xl bg-white shadow-2xl shadow-black/10 border border-[var(--color-line)] p-4 max-w-[260px]"
          >
          ...
          <motion.div
            ...
            className="hidden lg:block float absolute -left-8 bottom-24 z-10 rounded-2xl bg-white shadow-2xl shadow-black/10 border border-[var(--color-line)] p-4"
        ...
        >
          ...
          <Marquee speed={45} fade>
            {partners.map((p) => (
              <span
                key={p}
                className="font-semibold tracking-tight text-2xl text-[var(--color-fg-muted)]/70 whitespace-nowrap hover:text-[var(--color-fg)] transition-colors"
              >
                {p}
              </span>
            ))}
